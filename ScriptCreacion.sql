...
VALUES (1, N'd1b07fcb-0b94-42ca-bb02-a1f8d0f76a4f', N'Administrador', N'ADMINISTRADOR'),
(2, N'a4832290-f27f-4c02-b223-cb8ab677aee4', N'Usuario', N'USUARIO');
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'ConcurrencyStamp', N'Name', N'NormalizedName') AND [object_id] = OBJECT_ID(N'[AspNetRoles]'))
...
VALUES (1, NULL, N'dfchacon@uned.cr', N'DFCHACON@UNED.CR', N'DANICHAC', N'AQAAAAEAACcQAAAAEEyjzryIIDUE7UMMnLiTnVMFhYilwzuCh6NV2F09vzRjT343iq5jGv+fSCaC5na16w==', N'Chacón', N'Daniel', N'Navarro', N'', N'danichac');
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'ConcurrencyStamp', N'Email', N'NormalizedEmail', N'NormalizedUserName', N'password', N'primer_apellido', N'primer_nombre', N'segundo_apellido', N'segundo_nombre', N'nombre_usuario') AND [object_id] = OBJECT_ID(N'[AspNetUsers]'))
